...
function StepperTrigger({ asChild = false, className, children, tabIndex, ...props }: StepperTriggerProps) {
  ...
  return (
    <button
      ...
      className={cn(
        'cursor-pointer focus-visible:border-ring focus-visible:ring-ring/50 inline-flex items-center gap-3 rounded-full outline-none focus-visible:z-10 focus-visible:ring-[3px] disabled:pointer-events-none disabled:opacity-60 transition-all duration-300',
        className,
      )}
...
function StepperIndicator({ children, className }: React.ComponentProps<'div'>) {
  ...
  return (
    <div
      ...
      data-state={state}
      className={cn(
        'relative flex items-center overflow-hidden justify-center size-8 shrink-0 border-background bg-accent text-accent-foreground rounded-full text-sm data-[state=completed]:bg-primary data-[state=completed]:text-primary-foreground data-[state=active]:bg-primary data-[state=active]:text-primary-foreground transition-all duration-300',
        className,
      )}
    >
...
function StepperSeparator({ className, ...props }: React.ComponentProps<'div'>) {
  ...
  return (
    <div
      ...
      data-state={state}
      className={cn(
        'm-0.5 rounded-full bg-muted group-data-[orientation=vertical]/stepper-nav:h-12 group-data-[orientation=vertical]/stepper-nav:w-0.5 group-data-[orientation=horizontal]/stepper-nav:h-0.5 group-data-[orientation=horizontal]/stepper-nav:flex-1 transition-all duration-300',
        className,
      )}
      {...props}
    />
...
function StepperTitle({ children, className }: React.ComponentProps<'h3'>) {
  ...
  return (
    <h3 data-slot="stepper-title" data-state={state} className={cn('text-sm font-semibold transition-all duration-300', className)} style={{ fontFamily: 'var(--font-serif)', letterSpacing: '-0.02em' }}>
      {children}
    </h3>
  );
}
...
function StepperDescription({ children, className }: React.ComponentProps<'div'>) {
  ...
  return (
    <div data-slot="stepper-description" data-state={state} className={cn('text-sm text-muted-foreground transition-all duration-300', className)}>
      {children}
    </div>
  );
}
...
function StepperContent({ value, forceMount, children, className }: StepperContentProps) {
  ...
  return (
    <div
      ...
      data-state={activeStep}
      className={cn('w-full transition-all duration-500', className, !isActive && forceMount && 'hidden')}
